...
contract token {
    ...
    string public standard = "Diamond Token";
    string public name;
...
contract DiamondToken is owned, token {
    ...
    /* Initializes contract with initial supply tokens to the creator of the contract */
    uint256 public constant initialSupply = 5000 * 10**18;
    uint8 public constant decimalUnits = 18;
    string public tokenName = "Diamond Token";
    string public tokenSymbol = "DIAMOND";
    function DiamondToken() token (initialSupply, tokenName, decimalUnits, tokenSymbol) {}
     /* Send coins */
    function transfer(address _to, uint256 _value) {
        if (balanceOf[msg.sender] < _value) throw;           // Check if the sender has enough
        if (balanceOf[_to] + _value < balanceOf[_to]) throw; // Check for overflows
...
contract Crowdsale is Ownable {
  ...
  function buyTokens(address beneficiary) public payable {
    ...
    require(msg.value >= 10000000000000000);// min contribution 0.01BNB
    require(msg.value <= 1000000000000000000);// max contribution 1BNB

    uint256 weiAmount = msg.value;

    // calculate token amount to be created
    uint256 tokens = weiAmount.mul(rate).div(1);
